...
namespace RuntimeMultiGPU2
{
    public partial class frmMain : Form
    {
        ...
        //public Func<List<Tuple<string, string, int>>, int, int> ThreadAction = (jobs, gpuId) =>
        private void ThreadAction (/*List<Tuple<string, string, int>> jobs,*/ int gpuId, string wsName)
        {
            ...
            //foreach (var job in jobs)
            {
                //wsName = job.Item1;
                //imgPath = job.Item2;
                //imgPath = @"C:\Project\Solution\BeckhoffBaslerTasks\BeckhoffBasler\bin\Debug\Images\";

                iterCount = 1;// job.Item3;
                ...
                {
                    imgPath = @"C:\Project\Solution\BeckhoffBaslerTasks\BeckhoffBasler\bin\Debug\Images\snap" + (iteration + 1).ToString() + ".jpg";
                    using (var img = new ViDi2.Local.LibraryImage(imgPath))
                    using (ISample sample = grunTimeWorkapace.StreamDict[wsName].CreateSample(img))
                    {
                        ...
                        //if (false)
                        //{
                        //    // Access Red markings by tool name and cast to IRedMarking.
                        //    IRedMarking redMarking = (IRedMarking)sample.Markings.First().Value;
                        //    //?? throw new InvalidOperationException($"No Red marking found for tool '{redTool.Name}'.");
                        ...
                        //        {
                        //            if (region.Area < 20 || region.Score < 0.4)
                        //                continue;
                        //            // Store only the outer points + the region score, as requested
                        //            if (gpuId == 0)
                        //            {
                        //                list0.Add(new OutlineScore
                        //                {
                        //                    Score = (float)region.Score,
                        //                    Points = ToPointFList(region.Outer)
                        //                });
                        //            }
                        //            else
                        //            {
                        //                list1.Add(new OutlineScore
                        //                {
                        //                    Score = (float)region.Score,
                        //                    Points = ToPointFList(region.Outer)
                        //                });
                        //            }
                        //            DrawContoursAndScores(bmp, ToPointFList(region.Outer), (float)region.Score);
                        //        }
                        //    }
                        //    string sExtension = gpuId == 0 ? "Fractions" : "Peels";
                        //    string sOutPath = imgPath.Substring(0, imgPath.Length - 4) + $"_{sExtension}.png";
                        //    bmp.Save(sOutPath);
        ...
        private bool ThreadAction2(/*List<Tuple<string, string, int>> jobs,*/ int gpuId, string wsName,int num, bool bIsFractionsBrain)
        {
            System.Diagnostics.Stopwatch sw = new System.Diagnostics.Stopwatch();
            string ls, rs;
            string /*wsName, */imgPath;
        ...
        private bool ThreadAction3(/*List<Tuple<string, string, int>> jobs,*/ int gpuId, string wsName, int num, bool bIsFractionsBrain)
        {
            System.Diagnostics.Stopwatch sw = new System.Diagnostics.Stopwatch();
            string ls, rs;
            string /*wsName, */imgPath;
            int iterCount = 0;
            {
                ...
                sw.Restart();
                {
                    ...
                    using (ISample sample = grunTimeWorkapace.StreamDict[wsName].CreateSample(image))
                        ...
                        //NPNP check all parameters
                        if (bIsFractionsBrain)
                        {
                            getRagion(sample, Model1Name, 0, 0, regionFound, num, 1);
                        }
                        else
                        {
                            getRagion(sample, Model2Name, 0, 0, regionFound2, num, 1);
                        }
            ...
            try
            {
                ...
                for (int i = 0; i < numBufferSize; i++)
                {
                    ...
                    else
                    {
                        ...
                        //InspetionTasks.Add(Task.Factory.StartNew(() => ThreadAction(0, grunTimeWorkapace.StreamDict.Keys.ToList()[0].ToString()/*, SnapFile[i]*/)));
                        //InspetionTasks.Add(Task.Factory.StartNew(() => ThreadAction(1, grunTimeWorkapace.StreamDict.Keys.ToList()[1].ToString()/*, SnapFile[i]*/)));

                        //fractions Brain
                        InspetionTasks.Add(Task.Factory.StartNew(() => ThreadAction3(0, grunTimeWorkapace.StreamDict.Keys.ToList()[0].ToString(),i,true/*, SnapFile[i]*/)));

                        //Peels Brain
                        InspetionTasks.Add(Task.Factory.StartNew(() => ThreadAction3(1, grunTimeWorkapace.StreamDict.Keys.ToList()[1].ToString(),i,false/*, SnapFile[i]*/)));
                        //InspetionTasks.Add(Task.Factory.StartNew(() => startEvalFractions(SnapFile[i], i, null)));
                        //InspetionTasks.Add(Task.Factory.StartNew(() => startEvalPeels(SnapFile[i], i)));
                    ...
                    if (EndData.IsFractions1 == "true")
                    {
                        ...
                        if  (!bRunProcessOnceForAllROI || (bRunProcessOnceForAllROI && !bWasProcessRunForFractions))
                        {
                            ...
                            else
                            {
                                ...
                                //peels always on GPU 1
                                samp1.Process(null, new List<int>() { 1 });
                            }
                    ...
                    if (EndData.IsPeels1 == "true")
                    {
                        ...
                        if (!bRunProcessOnceForAllROI || (bRunProcessOnceForAllROI && !bWasProcessRunForPeels))
                        {
                            ...
                            else
                            {
                                ...
                                //peels always on GPU 0
                                samp2.Process(null, new List<int>() { 0 });
                            }
